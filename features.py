...
def add_features(df: pd.DataFrame, target_horizon: int) -> tuple[pd.DataFrame, list[str]]:
    ...
    df["vol_ratio_5_30"] = df["vol_5"] / df["vol_30"]

    # Optional boolean breakout flag
    ...
    for w in ma_windows:
        ...
        df[f"dist_ma_{w}"] = df["close"] / df[f"ma_{w}"] - 1.0

    # keep older naming too if you want compatibility
    df["close_over_ma_5"] = df["dist_ma_5"]
    df["close_over_ma_15"] = df["dist_ma_15"]
    df["close_over_ma_60"] = df["dist_ma_60"]

    dist_ma_15_mean_60 = df["dist_ma_15"].rolling(60).mean()
    dist_ma_15_std_60 = df["dist_ma_15"].rolling(60).std()
    df["dist_ma_15_z"] = (df["dist_ma_15"] - dist_ma_15_mean_60) / dist_ma_15_std_60
    ...
    df["range_ratio"] = df["range_5"] / df["range_15"]
    ...
    df["volume_z"] = (df["volume"] - df["volume_ma_20"]) / df["volume_std_20"]
    df["volume_zscore_20"] = df["volume_z"]

    df["trades_ma_20"] = df["num_trades"].rolling(20).mean()
    df["trades_std_20"] = df["num_trades"].rolling(20).std()
    df["trades_z"] = (df["num_trades"] - df["trades_ma_20"]) / df["trades_std_20"]

    # keep compatibility with previous names
    df["num_trades_ma_20"] = df["trades_ma_20"]
    df["num_trades_std_20"] = df["trades_std_20"]
    df["num_trades_zscore_20"] = df["trades_z"]
